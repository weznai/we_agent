...
def resolve_model_for_agent(
    ...
    logger.info(f"Resolving agent model: agent_type={agent_type}, model_id={model_id}")
    ...
    if model_id:
        ...
        if not mapping:
            raise ValueError(f"no model mapping for agent_type={agent_type}")
        model = db.query(Model).filter(
            Model.id == mapping.model_id, Model.is_active == True
        ).first()
    ...
    logger.info(
        f"Agent model resolved: model={model.name}, provider={provider.name}, api_base={provider.api_base}"
    )
    return model, provider

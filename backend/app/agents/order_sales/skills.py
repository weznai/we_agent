...
SKILL_FILE = Path(__file__).resolve().parent.parent.parent / "skills" / "order_sales_management.md"
...
def create_knowledge_search_tool(db, user_id: int, group_id: Optional[int] = None):
    @lc_tool
    def knowledge_search(query: str) -> str:
        ...
        try:
            ...
            results = knowledge_service.search_for_agent(user_id, query, group_id, top_k=5, db=db)
            if not results:
